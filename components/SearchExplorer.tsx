...
useEffect(() => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-8 flex min-h-[80vh] flex-col">
      {/* Search Bar Section */}
      <div className="relative">
        ...
        <div className="mt-3 flex gap-2 text-xs text-zinc-400">
          <span>Try:</span>
          <button
            onClick={() => setSearchQuery("Renan Peixoto")}
            className="hover:text-zinc-950 dark:hover:text-zinc-100 underline decoration-zinc-200 underline-offset-2 transition-colors"
          >
            Renan Peixoto
          </button>
          <button
            onClick={() => setSearchQuery("Frontend Developer")}
            className="hover:text-zinc-950 dark:hover:text-zinc-100 underline decoration-zinc-200 underline-offset-2 transition-colors"
          >
            Frontend Developer
          </button>
      ...
      <div className="space-y-4 flex-1">
        {debouncedQuery && (
          <div className="flex items-center justify-between border-b border-zinc-100 pb-2 dark:border-zinc-800">
            <h2 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">
              {isSearching && streamingResults.length === 0
                ? "Loading results..."
                : `${streamingResults.length} Results found`}
            </h2>
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-1">
          {streamingResults.map((user: User, idx: number) => (
            ...
            >
              <ProfileCard user={user} onSelect={handleSelectUser} />
            </div>
          ))}
          ...
          {!debouncedQuery && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="mt-2 max-w-xs text-sm text-zinc-500">
                Type a name, role, or department above to explore talent on
                Torre.ai.
              </p>
            </div>
          )}
